...
strTotalTax=request("txtTotalTax")

if strAmount=>strBalanceDue and strStatusFrom<>"Pending" then
  strStatusTo="Paid"
  strErrorMessage="Invoice was pending."
end if

if strAmount="" and strStatusTo="Paid" then
    strAmount=strBalanceDue
end if

if strAmount<strbalanceDue then 

  strPercentBeingPaid=strAmount/strInvoiceTotal
'response.write "%=" & strPercentBeingPaid
  strTax=strTotalTax*strPercentBeingPaid
  strReg=(strInvoiceTotal-strTotalTax)*strPercentBeingPaid
  response.write "apply to tax: $" & formatnumber(strTax,2) & " apply other: $" & formatnumber(strReg,2)
end if
...
'if strStatusTo="Paid" and strBalanceDue=0 then
'  strGo="No"
'  strErrorMessage="Action failed: Balance due is already zero"
'end if
...
'response.write strInput
  ...
  if strAmount>strBalanceDue then
    if strErrorMessage="Invoice was Pending." then
      response.write "<p class=ErrorMessage>Warning: Overpayment</p>"
      response.write "<p class=ErrorMessage>(Invoice was pending)</p>"
    else
      response.write "<p class=ErrorMessage>Warning: Overpayment</p>"
    end if
  end if

if strStatusFrom<>"Pending" then

  if strStatusFrom<> strStatusTo then
