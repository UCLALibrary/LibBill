...
'if location code is already in use, disallow update
...
if strGo<>"No" then
  ...
  hashURL=session("strBaseHash")
  hashURL=hashURL & "branches/edit_unit"
'response.write hashURL
  ...
  postURL=session("strBaseURL")
  postURL=postURL & "branches/edit_unit"
  ...
  if (xmlhttp.status >= 200) and (xmlhttp.status < 300) then
    response.write "<p class=GenericMessage>location updated:</p>"
  else
    strError=xmlhttp.responsetext
    response.write "<p>Status: " & strError & "</p>"
  end if
