...
public class EditLocationServiceProcedure
  extends StoredProcedure
{
  ...
  public EditLocationServiceProcedure( JdbcTemplate jdbcTemplate,
                                       String string )
  {
    super( jdbcTemplate, string );
  }

  public EditLocationServiceProcedure( DataSource dataSource,
                                       String string )
  {
    super( dataSource, string );
  }

  public EditLocationServiceProcedure()
  {
    super();
  }
  ...
  private void prepProc()
  {
    setDataSource( ds );
    setFunction( false );
    setSql( "update_location_service" );
    ...
    declareParameter( new SqlParameter( "p_service_name",
                                        Types.VARCHAR ) );
    declareParameter( new SqlParameter( "p_subtype_name",
                                        Types.VARCHAR ) );
    declareParameter( new SqlParameter( "p_unit_measure",
                                        Types.VARCHAR ) );
    ...
    declareParameter( new SqlParameter( "p_require_custom_price",
                                        Types.CHAR ) );
    declareParameter( new SqlParameter( "p_uc_price", Types.NUMERIC ) );
    declareParameter( new SqlParameter( "p_non_uc_price",
                                        Types.NUMERIC ) );
    declareParameter( new SqlParameter( "p_uc_minimum_amount",
                                        Types.NUMERIC ) );
    declareParameter( new SqlParameter( "p_non_uc_minimum_amount",
                                        Types.NUMERIC ) );
    compile();
  }
  ...
  private Map execute()
  {
    ...
    input.put( "p_location_service_id", getData().getLocSvcKey() );
    input.put( "p_user_name", getData().getWhoBy() );
    if ( getData().getLocationID() != -1 )
      input.put( "p_location_id", getData().getLocationID() );
    else
      input.put( "p_location_id", null );
    ...
    input.put( "p_require_custom_price",
               ( getData().isRequireCustomPrice() ? "Y": "N" ) );
